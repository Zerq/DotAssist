...
namespace WebApplication2 {
    ...
    public class DotNetCli {
        ...
        public List<ExpandoObject> GetTemplates(string version, TemplateType type) {
            ...
            var vers = this.GetVersions();
            if (vers == null){
                throw new ApplicationException("No versions found");
            }

            var first = vers.FirstOrDefault(n => n.Name == version);
            if (first == null){
                 throw new ApplicationException("No matching versions found for " + version);
            }

            foreach (var file in first.GetFiles()) {
